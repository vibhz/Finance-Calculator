...
class MortgageViewController: UIViewController, UITextFieldDelegate {
    ...
    func loadDefaultsData(_ historyKey :String) {
        ...
        mortgage.history = defaults.object(forKey: historyKey) as? [String] ?? [String]()
    }
    ...
    @IBAction func onSave(_ sender: UIBarButtonItem) {
        ...
        if loanAmountField.checkIfEmpty() == false && interestField.checkIfEmpty() == false && paymentField.checkIfEmpty() == false && numberOfYearsField.checkIfEmpty() == false {
            ...
            if(mortgage.totalLoan != 0 && mortgage.interestAmount != 0 && mortgage.totalPayment != 0 && mortgage.numberOfYears != 0) {
                
                let defaults = UserDefaults.standard
                let historyString = " 1. Mortgage Value - \(mortgage.totalLoan) \n 2. Interest Rate (%) - \(mortgage.interestAmount)  \n 3. Monthly Payment - \(mortgage.totalPayment) \n 4. Number of Years (Time) - \(mortgage.numberOfYears)"
                ...
                (mortgage.totalLoan, mortgage.interestAmount, mortgage.totalPayment, mortgage.numberOfYears) = (0,0,0,0)
                
                ///Makes sure that the history list only holds last 5 calculations
                if(mortgage.history.count > 4) {
                    mortgage.history.removeFirst()
                    mortgage.history.append(historyString)
                    defaults.set(mortgage.history, forKey: "MortgageHistory")
                } else {
                    mortgage.history.append(historyString)
                    defaults.set(mortgage.history, forKey: "MortgageHistory")
                }
    ...
    @IBAction func onCalculate(_ sender: UIButton) {
        ...
        if loanAmountField.checkIfEmpty() == false && interestField.checkIfEmpty() == false && paymentField.checkIfEmpty() == false && numberOfYearsField.checkIfEmpty() == true{
            ...
            mortgage.totalLoan = Double(loanAmountField.text!)!
            mortgage.interestAmount = Double(interestField.text!)!
            mortgage.totalPayment = Double(paymentField.text!)!
            
            numberOfYearsField.text = String(mortgage.numberOfYearsCalculation())
            ...
            paymentField.text = String(mortgage.monthlyPaymentCalculation())
            storeTextFieldValues()
            ...
            loanAmountField.text = String(mortgage.loanAmountCalculation())
            storeTextFieldValues()
            ...
            interestField.text = String(mortgage.AIR_Calculation())
            storeTextFieldValues()
