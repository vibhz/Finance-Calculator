...
class LoansViewController: UIViewController, UITextFieldDelegate {
    ...
    func loadDefaultsData(_ historyKey :String) {
        ...
        loan.history = defaults.object(forKey: historyKey) as? [String] ?? [String]()
    }
    ...
    @IBAction func onSave(_ sender: UIBarButtonItem) {
        ...
        if loanAmountField.checkIfEmpty() == false && interestField.checkIfEmpty() == false && paymentField.checkIfEmpty() == false && numberOfPaymentsField.checkIfEmpty() == false {
            ...
            if(loan.totalLoan != 0 && loan.interestAmount != 0 && loan.totalPayment != 0 && loan.numberOfPayments != 0) {
                
                let defaults = UserDefaults.standard
                let historyString = " 1. Loan Amount - \(loan.totalLoan) \n 2. Interest Rate (%) - \(loan.interestAmount) \n 3. Monthly Payment - \(loan.totalPayment) \n 4. Number of Payments - \(loan.numberOfPayments)"
                ...
                (loan.totalLoan, loan.interestAmount, loan.totalPayment, loan.numberOfPayments) = (0,0,0,0)
                
                ///Makes sure that the history list only holds last 5 calculations
                if(loan.history.count > 4) {
                    loan.history.removeFirst()
                    loan.history.append(historyString)
                    defaults.set(loan.history, forKey: "LoansHistory")
                } else {
                    loan.history.append(historyString)
                    defaults.set(loan.history, forKey: "LoansHistory")
                }
    ...
    @IBAction func onCalculate(_ sender: UIButton) {
        ...
        if loanAmountField.checkIfEmpty() == false && interestField.checkIfEmpty() == false && paymentField.checkIfEmpty() == false && numberOfPaymentsField.checkIfEmpty() == true{
            ...
            loan.totalLoan = Double(loanAmountField.text!)!
            loan.interestAmount = Double(interestField.text!)!
            loan.totalPayment = Double(paymentField.text!)!
            
            numberOfPaymentsField.text = String(loan.numberOfPaymentsCalculation())
            ...
            paymentField.text = String(loan.monthlyPaymentCalculation())
            storeTextFieldValues()
            ...
            loanAmountField.text = String(loan.loanAmountCalculation())
            storeTextFieldValues()
            ...
            interestField.text = String(loan.AIR_Calculation())
            storeTextFieldValues()
